...
class CandleStickStrategy:
    ...
    def predict_operation(self, prev_candle, last_candle, trend):
        """
        prev_candle y last_candle deben ser diccionarios:
        {
            'has_upper_wick': bool,
            'has_lower_wick': bool,
            'body': float,
        ...
        """
        print("prev_candle:", prev_candle)
        print("last_candle:", last_candle)
        print("trend:", trend)

        body_prev = prev_candle["body"]
        body_last = last_candle["body"]

        # ---------------------------
        # 1) Rebote por falta de mecha superior + caída fuerte → LONG
        # ---------------------------
        if (not last_candle["has_upper_wick"]
            and last_candle["has_lower_wick"]
            and last_candle["close"] < prev_candle["close"]):
            return "LONG"

        # ---------------------------
        # 2) Dos velas con cuerpo muy pequeño → SHORT
        # ---------------------------
        if body_prev < 0.00008 and body_last < 0.00008:
            return "SHORT"

        # ---------------------------
        # 3) Tendencia fuerte UP + cuerpo pequeño = agotamiento → SHORT
        # ---------------------------
        if trend == "UP" and body_last < body_prev and last_candle["has_upper_wick"]:
            return "SHORT"

        # ---------------------------
        # 4) Dos velas bajistas con mecha inferior grande → LONG
        # ---------------------------
        if (trend == "DOWN"
            and last_candle["has_lower_wick"]
            and prev_candle["has_lower_wick"]):
            return "LONG"

        # ---------------------------
        # 5) Cambio brusco de tendencia + mechas → reversión
        # ---------------------------
        if prev_candle["has_upper_wick"] and prev_candle["has_lower_wick"]:
            if trend == "DOWN":
                return "LONG"
            elif trend == "UP":
                return "SHORT"
    ...
    def get_signal_for_new_candle(self):
        ...
        # 1) Rebote por falta de mecha superior + caída fuerte → LONG
        # ---------------------------
        if (not has_upper_wick and has_lower_wick and close_price < low_price_prev):
            return "LONG"

        # ---------------------------
        # 2) Dos velas con cuerpo muy pequeño → SHORT
        # ---------------------------
        if body_prev < 0.00008 and body < 0.00008:
            return "SHORT"

        # ---------------------------
        # 3) Tendencia fuerte UP + cuerpo pequeño = agotamiento → SHORT
        # ---------------------------
        if trend == "UP" and body < body_prev and has_upper_wick:
            return "SHORT"

        # ---------------------------
        # 4) Dos velas bajistas con mecha inferior grande → LONG
        # ---------------------------
        if (trend == "DOWN" and has_lower_wick and has_lower_wick_prev):
            return "LONG"

        # ---------------------------
        # 5) Cambio brusco de tendencia + mechas → reversión
        # ---------------------------
        if has_upper_wick_prev and has_lower_wick_prev:
            if trend == "DOWN":
                return "LONG"
            elif trend == "UP":
                return "SHORT"
