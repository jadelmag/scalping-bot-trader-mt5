...
class BearishHaramiSimulator:
    ...
    @staticmethod
    def strategy_bearish_harami(symbol: str="EURUSD", volume: float=0.01, signal: str="NEUTRAL"):
        """
        ...
        if signal == LONG:
            BearishHaramiSimulator.open_long(symbol, volume)
        elif signal == SHORT:
            BearishHaramiSimulator.open_short(symbol, volume)
        else:
        ...
        BearishHaramiSimulator.monitor_positions(symbol)
    ...
    def open_long(symbol, volume, sl_pips=200, tp_pips=300):
        ...
        try:
            ...
            order = BearishHaramiOrder(symbol, "long", price_open, volume, sl_price, tp_price, position_id=result.order)
            BearishHaramiSimulator.open_positions.append(order)
    ...
    def open_short(symbol, volume, sl_pips=200, tp_pips=300):
        ...
        try:
            ...
            order = BearishHaramiOrder(symbol, "short", price_open, volume, sl_price, tp_price, position_id=result.order)
            BearishHaramiSimulator.open_positions.append(order)
    ...
    def clear_positions():
        ...
        BearishHaramiSimulator.open_positions = [pos for pos in BearishHaramiSimulator.open_positions if not pos.closed]
    ...
    def monitor_positions(symbol):
        ...
        try:
            while True:
                ...
                for order in BearishHaramiSimulator.open_positions:
                    if order.closed:
                        continue
                    ...
                    if order.profit > 0:
                        ...
                        if order.profit < close_profit:  
                            BearishHaramiSimulator.close_position(order)
                            BearishHaramiSimulator.clear_positions()
                            return;
                    elif seconds >= 59:
                        numTimes += 1
                        if (numTimes >= 4):
    ...
    def recovery_profit(lostProfit, currentType, order):
        ...
        oppositeType = BearishHaramiSimulator.get_opposite_type(currentType)
        if (oppositeType == "long"):
            BearishHaramiSimulator.open_long(order.symbol, order.price_open, order.volume * 2)
        elif (oppositeType == "short"):
            BearishHaramiSimulator.open_short(order.symbol, order.price_open, order.volume * 2)
